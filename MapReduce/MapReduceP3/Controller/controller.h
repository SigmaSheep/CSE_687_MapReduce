...
public:
	Controller(boost::asio::ip::tcp::socket socket,
	...
	void Start() {
		...
		io_service_.post([this]() {
			boost::asio::read(socket_,boost::asio::buffer(arguments_, 16));
			std::cout << "i read" << arguments_ << "\n"; });
			*/
	}; // end of Start function

private:
	void WaitMapFinish() {
		//size_t len = boost::asio::read(socket_,
			//boost::asio::buffer(arguments_, 16));
	};
	int stub_index_;
	int stub_count_;
...
public:
	...
	StubConnection(const std::vector<std::string>& ip_address_list, 
		...
		query_(ip_address_list[0], port_list[0]) {Connect(); }
	void Connect(){
		endpoint_iterator_ = resolver_.resolve(query_); //update endpoint
		boost::asio::connect(socket_, endpoint_iterator_);
		std::make_shared<Controller>(std::move(socket_), io_service_, strand_,
